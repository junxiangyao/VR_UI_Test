            ...
            if (right_controller.GetComponent<VRTK_ControllerEvents>().triggerPressed)
            {
            	if(hit.transform.gameObject.CompareTag("MiniMap"))
            	{
            		...
            		if(dist_mini <= 0.15f)
            		{
            			...
            			// Debug.Log("!!!!!!"+ hit_point.transform.InverseTransformPoint(transform.position));
            			// hit.point.InverseTransformPoint(transform.position)
            			// transform_buffer.transform.position = hit.point;
            			// Debug.Log(transform_buffer.transform.position);
            			// teleporter.ForceTeleport(transform_buffer.transform.InverseTransformPoint(transform.localPosition), 
